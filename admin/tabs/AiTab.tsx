...
const AiTab: React.FC<AiTabProps> = ({ settings, onUpdate }) => {
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="bg-[#1e2329] p-6 rounded-xl border border-[#2b3139] shadow-lg">
                <div className="flex items-center justify-between mb-6">
                    <div><h3 className="text-lg font-bold text-white">AI Analyst Configuration</h3></div>
                    <div className={`w-3 h-3 rounded-full ${settings.aiAnalyst.isEnabled ? 'bg-[#0ecb81]' : 'bg-[#474d57]'}`}></div>
                </div>
                <div className="flex items-center justify-between p-4 bg-[#131722] rounded-lg border border-[#2b3139] mb-6">
