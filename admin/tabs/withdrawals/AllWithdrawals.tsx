...
interface AllWithdrawalsProps {
  paymentRequests: PaymentRequest[];
  onProcessPayment: (reqId: string, action: 'APPROVED' | 'REJECTED') => void;
  onViewProof: (url: string) => void;
}

const AllWithdrawals: React.FC<AllWithdrawalsProps> = ({ paymentRequests, onProcessPayment, onViewProof }) => {
  const withdrawals = useMemo(
    () => paymentRequests.filter((req) => req.type === 'WITHDRAWAL'),
    [paymentRequests]
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="flex items-center justify-between rounded-2xl border border-emerald-100 bg-emerald-50/60 px-4 py-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-xs font-medium text-slate-500">Approved Withdrawals</div>
              <div className="text-sm font-semibold text-slate-900">
                ${totalApproved.toFixed(2)} USD
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-rose-100 bg-rose-50/60 px-4 py-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-xs font-medium text-slate-500">Rejected Withdrawals</div>
              <div className="text-sm font-semibold text-slate-900">
                ${totalRejected.toFixed(2)} USD
              </div>
            </div>
          </div>
        </div>
              ...
              {withdrawals.map((req) => (
                <tr key={req.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 align-top">
                    ...
                    {req.status === 'APPROVED' && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold border border-emerald-300 text-emerald-600 bg-emerald-50">
                        Completed
                      </span>
                    )}
                    {req.status === 'REJECTED' && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold border border-rose-300 text-rose-600 bg-rose-50">
                        Rejected
                      </span>
                    )}
                  </td>
