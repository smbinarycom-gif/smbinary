...
interface RejectedWithdrawalsProps {
  paymentRequests: PaymentRequest[];
  onProcessPayment: (reqId: string, action: 'APPROVED' | 'REJECTED') => void;
  onViewProof: (url: string) => void;
}

const RejectedWithdrawals: React.FC<RejectedWithdrawalsProps> = ({ paymentRequests, onProcessPayment, onViewProof }) => {
  const withdrawals = useMemo(
    () => paymentRequests.filter((req) => req.type === 'WITHDRAWAL' && req.status === 'REJECTED'),
    [paymentRequests]
  );
                ...
                <tr key={req.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 align-top text-right whitespace-nowrap">
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center px-3 py-1.5 rounded-full bg-emerald-500 text-white text-[11px] font-semibold hover:bg-emerald-600"
                    >
                      Approve
                    </button>
                  </td>
