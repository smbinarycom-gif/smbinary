...
interface WithdrawalsMenuProps {
  activeMenu: string;
  onSelect: (menu: string) => void;
  withdrawalCounts?: Record<string, number>;
  theme?: AdminThemeSettings;
}

const WithdrawalsMenu: React.FC<WithdrawalsMenuProps> = ({ activeMenu, onSelect, withdrawalCounts, theme }) => {
  ...
  const ITEMS = [
    { id: 'WITHDRAWALS_ALL', label: 'All Withdrawals', iconClass: 'fa-solid fa-list-ul' },
    { id: 'WITHDRAWALS_PENDING', label: 'Pending Withdrawals', iconClass: 'fa-solid fa-hourglass-half' },
    { id: 'WITHDRAWALS_APPROVED', label: 'Approved Withdrawals', iconClass: 'fa-solid fa-circle-check' },
    { id: 'WITHDRAWALS_REJECTED', label: 'Rejected Withdrawals', iconClass: 'fa-solid fa-circle-xmark' },
  ];

  const getBadgeClasses = (id: string) => {
    switch (id) {
      case 'WITHDRAWALS_ALL':
        // Soft blue pill like screenshot
    ...
    <div className="mt-1">
      ...
      >
        <span className="flex items-center space-x-3.5">
          ...
          <span className="text-[13px] font-semibold font-binance tracking-wide">Withdrawals</span>
        </span>
      ...
      {isOpen && (
        <div className={`pl-8 pr-4 pb-2 pt-1 space-y-0.5 ${submenuContainerClasses}`}>
          {ITEMS.map((item) => {
            ...
            const count = withdrawalCounts?.[item.id] ?? 0;
            return (
              <button
                key={item.id}
